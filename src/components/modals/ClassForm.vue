<template>
    <div :class="['classFormWrapper', { 'inDialog': isEdit }]">
        ...
        <div class="addClassForm">
            ...
            <p v-else-if="!isEdit && !schoolAdminSchools.length" class="formError">
...
watch(
    () => props.preferredSchoolId,
    (sid) => {
        if (!isEdit.value && sid) {
            if (isSchoolAdminCreate.value) schoolAdminSchoolId.value = sid;
            else teacherOptionalSchoolId.value = sid;
        }
    },
    { immediate: true }
);
